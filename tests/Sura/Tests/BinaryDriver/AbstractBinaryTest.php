...
use Sura\BinaryDriver\Exception\ExecutableNotFoundException;
...
class AbstractBinaryTest extends BinaryDriverTestCase
{
    ...
    public function testSimpleLoadWithBinaryPath()
    {
        ...
        $this->assertInstanceOf('Sura\Tests\BinaryDriver\Implementation', $imp);
    ...
    {
        $conf = $this->getMock('Sura\BinaryDriver\ConfigurationInterface');
        $imp = Implementation::load('php', null, $conf);
    ...
    {
        $imp = Implementation::load('php');
        $factory = $this->getMock('Sura\BinaryDriver\ProcessBuilderFactoryInterface');
    ...
    public function testListenRegistersAListener()
    {
        ...
        $listeners = $this->getMockBuilder('Sura\BinaryDriver\Listeners\Listeners')
            ->disableOriginalConstructor()
            ->getMock();

        $listener = $this->getMock('Sura\BinaryDriver\Listeners\ListenerInterface');

        $listeners->expects($this->once())
            ->method('register')
            ->with($this->equalTo($listener), $this->equalTo($imp));

        $reflexion = new \ReflectionClass('Sura\BinaryDriver\AbstractBinary');
    ...
    public function testCommandRunsAProcess($parameters, $bypassErrors, $expectedParameters, $output)
    {
        $imp = Implementation::load('php');
        $factory = $this->getMock('Sura\BinaryDriver\ProcessBuilderFactoryInterface');
        $processRunner = $this->getMock('Sura\BinaryDriver\ProcessRunnerInterface');
